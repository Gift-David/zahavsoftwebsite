"use client"
import { useForm } from '@formspree/react';
import { useEffect, useState, useRef } from "react";

export default function ContactForm() {
    const service_option = [
        'Logo Design',
        'Brand Identity Design',
        'Social Media Designs',
        'Website Design',
        'Mobile App Design',
        'Web Development',
        'Mobile App Development',
    ]
    const formRef = useRef<HTMLFormElement>(null);
    const [isOpen, setIsOpen] = useState(false)

    const [state, handleSubmit, reset] = useForm("mlgzjbkd");

    useEffect (() => {
        if (state.succeeded) {
            reset();
            setIsOpen(true);
            formRef.current?.reset(); 
            reset();
    }
    }, [reset, state.succeeded]);

    


    return (
        <>
            <section id="contact-form" className='scroll-mt-20'>
                <div className="flex flex-col gap-10 bg-background border border-foreground pt-4 pl-4 pr-4 pb-2 md:pt-12 md:pb-12 md:pl-8 md:pr-8">
                    <h4 className="header-title text-[18px] md:text-2xl text-left">Send Us A Message</h4>
                    <form ref={formRef} action="" onSubmit={handleSubmit} className="flex flex-wrap gap-4 md:gap-8">
                        <div className="flex flex-col gap-2 min-w-full md:min-w-[47%]">
                            <label htmlFor="first_name">First Name</label>
                            <input name='first_name' type="text" required placeholder="Your first name" />
                        </div>

                        <div className="flex flex-col gap-2 min-w-full md:min-w-[47%]">
                            <label htmlFor="last_name">Last Name</label>
                            <input name='last_name' type="text" required placeholder="Your last name"/>
                        </div>

                        <div className="flex flex-col gap-2 min-w-full md:min-w-[47%]">
                            <label htmlFor="number">Phone Number</label>
                            <input name='phone_number' type="text" required placeholder="Your phone number"/>
                        </div>

                        <div className="flex flex-col gap-2 min-w-full md:min-w-[47%]">
                            <label htmlFor="email">Email Address</label>
                            <input name='email' type="email" required placeholder="Your email address"/>
                        </div>

                        <div className="flex flex-col gap-2 min-w-full md:min-w-full">
                            <label htmlFor="service">Service You Are Interested In</label>
                            <select name="service" id="" className=" service-select w-full font-secondary text-secondary-foreground p-4 border border-foreground" required>
                                <option value="" disabled selected className='text-placeholder bg-secondary-background'>Choose a Service</option>
                                {
                                    service_option.map((option, index) => (
                                        <option key={index} value={option} className=" text-base bg-secondary-background">{option}</option>
                                    ))
                                }

                                <option value="other" className=" text-base bg-secondary-background">Not sure yet, lets talk</option>
                                
                            </select>
                        </div>
                        
                        <div className="flex flex-col gap-2 min-w-full md:min-w-full">
                            <label htmlFor="organization">Company/Organization (optional)</label>
                            <input name='organization' type="text" placeholder="Your company name" />
                        </div>
                        
                        <div className="flex flex-col gap-2 min-w-full md:min-w-full">
                            <label htmlFor="">Tell Us About Your Project</label>
                            <textarea name="message" id="" placeholder="Tell us what your project is about" required className="w-full h-28"></textarea>
                        </div>
                  
                        <button className={`primary-btn w-full ${state.submitting? "cursor-not-allowed bg-foreground/70" : ""}`} type="submit" disabled={state.submitting}>{state.submitting? "Sending..." : "Send Message"}</button>
                        <p className="font-secondary text-secondary-foreground text-sm">We respond to enquiries within 6 hours . Your information will be never shared with third parties.</p>
                    </form>
                </div>
                
                
                {/* Success Modal */}
                <div className={`inset-0 z-20 justify-center items-center align-center bg-background/70 flex ${isOpen? "fixed" : "hidden"}`}>
                    <div className="flex flex-col gap-4 p-8 justify-center items-center align-center top-[50%] w-[80%] md:w-[70%] lg:w-[40%] h-[80%] bg-background">
                        <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M43.415 24.585C43.601 24.7707 43.7485 24.9913 43.8491 25.2341C43.9498 25.4769 44.0016 25.7372 44.0016 26C44.0016 26.2628 43.9498 26.5231 43.8491 26.7659C43.7485 27.0087 43.601 27.2293 43.415 27.415L29.415 41.415C29.2293 41.601 29.0087 41.7485 28.7659 41.8491C28.5231 41.9498 28.2628 42.0016 28 42.0016C27.7372 42.0016 27.4769 41.9498 27.2341 41.8491C26.9913 41.7485 26.7708 41.601 26.585 41.415L20.585 35.415C20.2097 35.0397 19.9989 34.5307 19.9989 34C19.9989 33.4693 20.2097 32.9603 20.585 32.585C20.9603 32.2097 21.4693 31.9989 22 31.9989C22.5307 31.9989 23.0397 32.2097 23.415 32.585L28 37.1725L40.585 24.585C40.7708 24.399 40.9913 24.2515 41.2341 24.1509C41.4769 24.0502 41.7372 23.9984 42 23.9984C42.2628 23.9984 42.5231 24.0502 42.7659 24.1509C43.0087 24.2515 43.2293 24.399 43.415 24.585ZM58 32C58 37.1423 56.4751 42.1691 53.6182 46.4448C50.7613 50.7205 46.7007 54.053 41.9498 56.0209C37.1989 57.9887 31.9712 58.5036 26.9277 57.5004C21.8842 56.4972 17.2514 54.0209 13.6152 50.3848C9.97907 46.7486 7.50281 42.1159 6.49959 37.0723C5.49638 32.0288 6.01127 26.8011 7.97914 22.0502C9.94702 17.2994 13.2795 13.2387 17.5552 10.3818C21.8309 7.52487 26.8577 6 32 6C38.8934 6.00728 45.5024 8.74889 50.3767 13.6233C55.2511 18.4976 57.9927 25.1066 58 32ZM54 32C54 27.6488 52.7097 23.3953 50.2923 19.7775C47.875 16.1596 44.439 13.3398 40.419 11.6747C36.3991 10.0095 31.9756 9.57385 27.708 10.4227C23.4404 11.2716 19.5204 13.3669 16.4437 16.4437C13.3669 19.5204 11.2716 23.4404 10.4227 27.708C9.57386 31.9756 10.0095 36.3991 11.6747 40.419C13.3398 44.439 16.1596 47.8749 19.7775 50.2923C23.3953 52.7097 27.6488 54 32 54C37.8327 53.9934 43.4247 51.6734 47.5491 47.549C51.6734 43.4247 53.9934 37.8327 54 32Z" fill="#FFF49B"/>
                        </svg>
                        <h2 className='text-center font-primary text-2xl text-secondary-foreground'>Message Received <br /> We'll Be In Touch</h2>
                        <p className='text-center font-secondary text-base text-secondary-foreground'>Thank you for reaching out to ZAHAVSOFT.  Your message has been received and a member of our team will respond within</p>
                        <p className='text-center font-primary text-2xl text-foreground'>6 HOURS</p>
                        <p className='text-center font-secondary text-base text-secondary-foreground'>In the meantime, feel free to explore our services or follow us on social media.</p>
                        
                        <a href="/">
                            <button className='primary-btn w-full'>Back To Home Page</button>
                        </a>
                        <button onClick={ () => setIsOpen(false)} className='text-secondary-foreground font-secondary underline underline-offset-4'>Close this window</button>
                    </div>
                </div>
            </section>
        </>
    )
}